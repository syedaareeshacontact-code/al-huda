...
import { useGlobalQuranAudioController } from '@/components/providers/global-quran-audio-provider';
...
} from '@/lib/quran-audio-sync';
import {
  buildQuranWordAudioFallbackUrl,
  type SurahWordAudioPayload,
...
type WordAudioLookup = Record<string, string>;
...
function getWeightedAyahIndex(progress: number, progressEnds: number[]) {
  if (progressEnds.length === 0) {
    return 0;
  }

  const target = clampRange(progress, 0, 0.999999);
  let low = 0;
  let high = progressEnds.length - 1;

  while (low < high) {
    const middle = Math.floor((low + high) / 2);
    if (target < progressEnds[middle]) high = middle;
    else low = middle + 1;
  }
...
function buildWordAudioKey(ayahNumber: number, wordIndex: number) {
  return `${ayahNumber}:${wordIndex}`;
}
...
const AudioSyncedArabicText = memo(function AudioSyncedArabicText({
  text,
  query,
  ayahNumber,
  activeWordAudioKey,
  ...
  query: string;
  ayahNumber: number;
  activeWordAudioKey: string | null;
  onWordAudioClick: (ayahNumber: number, wordIndex: number) => void;
}) {
  const words = getQuranRecitationTokens(text);

  return (
    <>
      {words.map(({ text: word, wordIndex }, index) => {
        const wordAudioKey = wordIndex ? buildWordAudioKey(ayahNumber, wordIndex) : null;
        const isWordAudioActive = Boolean(
          wordAudioKey && activeWordAudioKey === wordAudioKey
        );

        return (
          <span key={`${ayahNumber}-${index}-${word}`}>
            {wordIndex ? (
              <span
                role="button"
                tabIndex={0}
                id={`audio-word-${ayahNumber}-${wordIndex}`}
                data-audio-word={`${ayahNumber}:${wordIndex}`}
                data-word-audio="true"
                data-word-audio-active={isWordAudioActive ? 'true' : undefined}
                className="audio-synced-word"
                dir="rtl"
                lang="ar"
                onClick={() => onWordAudioClick(ayahNumber, wordIndex)}
                onKeyDown={(event) => {
                  ...
                  onWordAudioClick(ayahNumber, wordIndex);
                }}
                aria-label={`Play word audio: ${word}`}
                title="Play word audio"
              >
                <HighlightText text={word} query={query} />
              </span>
            ) : (
              <span className="audio-synced-word" dir="rtl" lang="ar">
                <HighlightText text={word} query={query} />
              </span>
            )}
...
export default function QuranReaderPage({
  ...
  const wordAudioRequestIdRef = useRef(0);
  const urduAyahNumberRef = useRef<number | null>(null);
  const shouldContinueUrduPlaylistRef = useRef(false);
  ...
  useEffect(() => {
    ...
    const clearActiveWordAudio = () => {
      setActiveWordAudioKey(null);
    };

    audio.addEventListener('ended', clearActiveWordAudio);
    audio.addEventListener('error', clearActiveWordAudio);

    return () => {
      audio.removeEventListener('ended', clearActiveWordAudio);
      audio.removeEventListener('error', clearActiveWordAudio);
      audio.pause();
  ...
  useEffect(() => {
    ...
    wordAudioRequestIdRef.current += 1;
    wordAudioRef.current?.pause();
    setActiveWordAudioKey(null);
    setWordAudioByKey({});

    const loadWordAudio = async () => {
      try {
        const payload = await fetchSurahWordAudio(surahId, controller.signal);
        if (!controller.signal.aborted) {
          setWordAudioByKey(buildWordAudioLookup(payload));
        }
      } catch (loadError) {
        const errorObject = loadError as { name?: string };
  ...
  const handleWordAudioClick = useCallback(
    (ayahNumber: number, wordIndex: number) => {
      if (!Number.isInteger(ayahNumber) || !Number.isInteger(wordIndex)) {
        return;
      }
      ...
      const fullSurahAudio = audioRef.current;
      if (fullSurahAudio && !fullSurahAudio.paused) {
        fullSurahAudio.pause();
      }

      const wordAudioKey = buildWordAudioKey(ayahNumber, wordIndex);
      const source =
        wordAudioByKey[wordAudioKey] ??
        buildQuranWordAudioFallbackUrl(surahId, ayahNumber, wordIndex);
      const requestId = wordAudioRequestIdRef.current + 1;
      wordAudioRequestIdRef.current = requestId;

      wordAudio.pause();
      wordAudio.src = source;
      wordAudio.currentTime = 0;
      wordAudio.load();
      setActiveWordAudioKey(wordAudioKey);

      wordAudio.play().catch(() => {
        if (wordAudioRequestIdRef.current !== requestId) {
          return;
        }
        ...
        showReaderActivity(
          {
            title: 'Word audio unavailable',
            description: `Ayah ${surahId}:${ayahNumber} word ${wordIndex} could not be played.`,
          },
          wordAudioKey
        );
      });
    },
    [audioRef, showReaderActivity, surahId, wordAudioByKey]
  );
  ...
  const weightedAyahProgressEnds = useMemo(() => {
    const weights = ayahs.map(({ translation }) =>
      Math.max(1, translation?.trim().length ?? 0)
    );
    ...
    return weights.map((weight) => {
      accumulated += weight;
      return totalWeight > 0 ? accumulated / totalWeight : 1;
    });
  }, [ayahs]);
  ...
  useEffect(() => {
    ...
    setUrduAyahNumber(null);
    urduAyahNumberRef.current = null;
    shouldContinueUrduPlaylistRef.current = false;
  ...
  useEffect(() => {
    ...
    const loadAudioSource = async () => {
      ...
      if (settings.audioPreference === 'tr') {
        const nextAyahNumber = urduAyahNumberRef.current ?? 1;
        setAudioReciters([]);
        setSelectedReciter(0);
  ...
  useEffect(() => {
    ...
    const loadTimings = async () => {
      try {
        ...
        const parsed = parseChapterTimings(payload.audio_file?.timestamps);
        setAyahTimings(parsed.ayahs);
  ...
  useEffect(() => {
    ...
    const shouldStartAudio =
      ...
      (settings.audioPreference === 'tr' && shouldContinueUrduPlaylistRef.current);
    shouldContinueUrduPlaylistRef.current = false;
  ...
  }, [audioRef, audioSrc, settings.audioPreference, settings.autoPlayAudio]);
    ...
    const onEnd = () => {
      if (settings.audioPreference === 'tr') {
        const nextAyahNumber = getNextUrduAyahNumber(
          ...
          surahDetail?.numberOfAyahs ?? ayahs.length
        );

        if (nextAyahNumber) {
          shouldContinueUrduPlaylistRef.current = true;
          setUrduAyahNumber(nextAyahNumber);
          setAudioSrc(buildUrduAyahAudioUrl(surahId, nextAyahNumber));
          setIsPlaying(false);
          setIsPlayPending(true);
  ...
  }, [audioRef, ayahs.length, loading, settings.audioPreference, surahDetail?.numberOfAyahs, surahId]);
  ...
  const startCurrentAudio = useCallback(() => {
    ...
    if (!audio.src || (audio.currentSrc && audio.currentSrc !== audioSrc)) {
      audio.src = audioSrc;
      audio.load();
    }
  ...
  useEffect(() => {
    ...
    const clearActiveWord = () => {
      activeAudioWordElementRef.current?.removeAttribute('data-audio-active');
      activeAudioWordElementRef.current = null;
    };

    const commitActiveAyah = (ayahNumber: number | null) => {
      if (activeAudioAyahNumberRef.current === ayahNumber) return;
      activeAudioAyahNumberRef.current = ayahNumber;
      setActiveAudioAyahNumber(ayahNumber);
    };

    if (!audio || !isPlaying || ayahs.length === 0) {
      clearActiveWord();
      commitActiveAyah(null);
      return;
    }

    if (settings.audioPreference === 'tr') {
      clearActiveWord();
      commitActiveAyah(urduAyahNumber ?? null);
      return;
    }
    ...
    const activateWord = (timing: WordTimingRange | null) => {
      const elementId = timing
        ...
        : null;
      const currentElement = activeAudioWordElementRef.current;
      ...
      if (nextElement) {
        nextElement.setAttribute('data-audio-active', 'true');
        activeAudioWordElementRef.current = nextElement;
      }
    };

    const syncHighlight = () => {
      const currentMs = Math.max(audio.currentTime || 0, 0) * 1000;
      const matchedAyah = findTimingAtMs(ayahTimings, currentMs, 100);
      let nextAyahNumber = matchedAyah?.ayahNumber ?? null;

      if (!nextAyahNumber) {
        const duration = audioDurationRef.current || getAudioDuration(audio);
        if (duration > 0) {
          const progress = (audio.currentTime || 0) / duration;
          const ayahIndex = getWeightedAyahIndex(progress, weightedAyahProgressEnds);
          nextAyahNumber = ayahs[ayahIndex]?.ayah.numberInSurah ?? null;
        }
      }
                      ...
                      >
                        ...
                        >
                          <AudioSyncedArabicText
                            ...
                            query={highlightQuery}
                            ayahNumber={ayah.numberInSurah}
                            activeWordAudioKey={activeWordAudioKey}
                            onWordAudioClick={handleWordAudioClick}
                          />
                          <AyahEndMarker
